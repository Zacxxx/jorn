    ...
    >
      ...
      <div className="mt-auto">
        <div className="pt-2 mt-1.5 border-t-2 border-slate-600/80 text-xs flex justify-between items-center font-semibold">
          ...
          <span className={`${spell.damageType === 'HealingSource' ? 'text-green-300' : spell.damage > 0 ? 'text-red-300' : 'text-slate-400'} flex items-center`}>
            {spell.damageType === 'HealingSource' ? <HealIcon className="inline-block w-4 h-4 mr-1 opacity-80"/> : spell.damage > 0 ? <SwordSlashIcon className="inline-block w-4 h-4 mr-1 opacity-80"/> : null}
            {spell.damageType === 'HealingSource' ? 'Heal:' : spell.damage > 0 ? 'Dmg:' : 'Effect'} <span className="font-bold text-lg ml-1">{spell.damage > 0 ? spell.damage : ''}</span>
          </span>
        </div>
