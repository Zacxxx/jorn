...
function validateResourceCosts(costs: any[] | undefined, maxTypes = 3, maxQuantity = 5): ResourceCost[] | undefined {
    if (costs && Array.isArray(costs)) {
        const validatedCosts = costs
            .filter((cost: any) => {
                if (typeof cost.itemId !== 'string' ||
                    typeof cost.quantity !== 'number' || !(cost.quantity > 0 && cost.quantity <= maxQuantity) ||
                    typeof cost.type !== 'string') {
                    return false;
                }
                // At this point, cost.type is definitely a string.
                return AVAILABLE_RESOURCE_TYPES_FOR_AI.includes(cost.type as ResourceType);
            })
            .slice(0, maxTypes)
            .map(cost => ({ // cost is any here, but filtered
                itemId: cost.itemId as string,
                quantity: cost.quantity as number,
                type: cost.type as ResourceType, 
            }));
...
- manaCost: (integer) Balanced mana cost (1-100). Consider level, components, configurations, pre-committed resources.
...
- resourceCost: (array of ResourceCost objects | null) List *ONLY ADDITIONAL* resources YOU determine are needed for balance. Use itemId from master list. ResourceType from: ${AVAILABLE_RESOURCE_TYPES_FOR_AI.join(', ')}. Quantity: 1-5. Max 3 types. DO NOT re-list the pre-committed resources.
...
`;
  ...
  try {
    ...
    validatedData.scalingFactor = getScalingFactorFromRarity(validatedData.rarity);
...
The 'resourceCost' field in your output should represent the NEW TOTAL resource cost for the refined spell. Use itemId from master list, type from ${AVAILABLE_RESOURCE_TYPES_FOR_AI.join(', ')}. Quantity 1-5, max 3 types.
                    ...
                    if (item.type && typeof item.type === 'string') { 
                         ...
                         if (AVAILABLE_RESOURCE_TYPES_FOR_AI.includes(typeStr as ResourceType)) {
                            mappedItem.type = typeStr as ResourceType;
                             ...
                             console.warn(`Invalid resource type '${typeStr}' in component researchRequirements.items. Omitting type.`);
                         }
                    }
                    return mappedItem;
                })
                .filter((item: Partial<ResourceCost>) => item.itemId && item.quantity && item.type) as Partial<ResourceCost>[]; // Ensure type is present
