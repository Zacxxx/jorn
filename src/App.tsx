...
import { INITIAL_PLAYER_STATS, STARTER_SPELL, ENEMY_DIFFICULTY_XP_REWARD, MAX_SPELLS_PER_LEVEL_BASE, PREPARED_SPELLS_PER_LEVEL_BASE, PREPARED_ABILITIES_PER_LEVEL_BASE, FIRST_TRAIT_LEVEL, TRAIT_LEVEL_INTERVAL, DEFAULT_QUEST_ICON, DEFAULT_TRAIT_ICON, INITIAL_PLAYER_INVENTORY, AVAILABLE_RESOURCE_TYPES_FOR_AI, BATTLE_RESOURCE_REWARD_TYPES, RESOURCE_ICONS, STATUS_EFFECT_ICONS, PLAYER_BASE_SPEED_FROM_REFLEX, INITIAL_PLAYER_EP, PLAYER_EP_REGEN_PER_TURN, STARTER_ABILITIES, PLAYER_BASE_BODY, PLAYER_BASE_MIND, PLAYER_BASE_REFLEX, HP_PER_BODY, HP_PER_LEVEL, BASE_HP, MP_PER_MIND, MP_PER_LEVEL, BASE_MP, EP_PER_REFLEX, EP_PER_LEVEL, BASE_EP, SPEED_PER_REFLEX, PHYSICAL_POWER_PER_BODY, MAGIC_POWER_PER_MIND, DEFENSE_PER_BODY, DEFENSE_PER_REFLEX, INITIAL_PLAYER_NAME, DEFENDING_DEFENSE_BONUS_PERCENTAGE, INITIAL_PLAYER_GOLD, INITIAL_PLAYER_ESSENCE, INITIAL_PLAYER_LOCATION, DEFAULT_SILENCE_DURATION, DEFAULT_ROOT_DURATION, AVAILABLE_SPELL_ICONS } from '../constants';
import { editSpell, generateEnemy, generateTrait, generateMainQuestStory, generateConsumable, generateEquipment, generateSpellFromDesign, generateSpellComponentFromResearch, generateLootFromChest, discoverRecipeFromPrompt } from '../services/geminiService';
...
export const App: React.FC<{}> = (): React.ReactElement => {
  ...
  const fetchInitialMainQuestIfNeeded = useCallback(async () => {
    ...
    if (!hasMainQuest && player.level >= 1 && !isLoading) {
      ...
      try {
        ...
        addLog('System', `New Quest: ${newQuest.title}`, 'info');
  ...
  }, [player.level, player.quests, isLoading, addLog]); 
  ...
  const handleFinalizeSpellDesign = async (
    ...
    try {
        ...
        const baseAndPlayerInvestedResources = designData.investedResources;
        const aiAddedResources = spellDetailsFromAI.resourceCost || [];
        const finalTotalResourceCosts = mergeResourceCosts(baseAndPlayerInvestedResources, aiAddedResources);

        const finalSpellData = {
            ...spellDetailsFromAI,
            resourceCost: finalTotalResourceCosts, 
  ...
  const handleInitiateSpellRefinement = async (originalSpell: Spell, refinementPrompt: string, augmentLevel?: number, selectedComponentId?: string) => {
    ...
    try {
      ...
      if (selectedComponentId) {
        const component = player.discoveredComponents.find(c => c.id === selectedComponentId);
        if (component) {
  ...
  }, [player.hp, player.ep, player.activeStatusEffects, effectivePlayerStats, addLog, player]);

  const awardCombatRewards = useCallback((defeatedEnemy: Enemy) => {
  ...
  }, [addLog, player.inventory, player.items, player.gold, player.essence, player.xp, player.bestiary, setPlayer]);
  ...
  }, [addLog, awardCombatRewards, currentEnemies, targetEnemyId, setGameState, setModalContent, setTargetEnemyId]);
 ...
 const processEnemyTurn = useCallback(() => {
    ...
    // let enemyIsRooted = false; // Not used for enemy turn logic currently
    let newEnemyActiveEffects: ActiveStatusEffect[] = [];
    actingEnemy.activeStatusEffects.forEach(effect => {
        // let effectPersists = true; // This variable is not used
        switch(effect.name) {
            case 'PoisonDoTActive': case 'BurningDoTActive': case 'BleedingDoTActive': case 'CorruptedDoTActive': case 'FrostbittenDoTActive': case 'RottingDoTActive': case 'ShockedDoTActive':
                if (effect.magnitude) {
            ...
            // case 'Rooted': enemyIsRooted = true; break; // Root doesn't prevent attacks
        }
      if (effect.duration - 1 <= 0) {
        addLog('Enemy', `${effect.name} on ${actingEnemy.name} has worn off.`, 'status');
        // effectPersists = false; // Not used
      } else {
  ...
  }, [currentEnemies, player, effectivePlayerStats, addLog, turn, currentActingEnemyIndex, handleEnemyDefeat, processPlayerTurnStartEffects]);
  ...
  }, [player.level, player.xp, player.xpToNextLevel, player.traits.length, addLog, calculateEffectiveStats, player]);
        ...
        for (const drop of lootDrops) {
            switch(drop.type) {
                ...
                case 'essence':
                    ...
                    break;
                case 'resource':
                    if (drop.itemId && drop.quantity && drop.resourceTypeName) { 
                        newInventory[drop.itemId] = (newInventory[drop.itemId] || 0) + drop.quantity;
                        foundMessages.push(`${drop.quantity}x ${drop.resourceTypeName}`);
                    }
                    break;
                case 'consumable':
